...
def another_graph():      
    ...
    column1,column2=st.columns(2)
    with column1:
        income_column=st.selectbox('1. 선택한 항목별 연 소득의 분포를 상자 그림으로 나타냅니다.',('성별', '자동차', '부동산', '자녀_수', '소득_형태', '학력', '가족_형태', '주거_형태', '연령대',  '직업', '가족_규모'))
        ...
        ('성별','자동차', '부동산','직장_전화','집_전화','이메일',
        '소득_형태', '학력', '가족_형태', '주거_형태', '직업',
        '자녀_수','가족_규모','신용카드_유효기간','나이'))
        if bar_column1 == '신용카드_유효기간' or bar_column1 == '나이' or bar_column1 == '가족_규모' or bar_column1 == '자녀_수':
            ...
            for col1, col2 in [[bar_column1, '고용연수']]:
                ...
                st.plotly_chart(fig)
    with column2:
        income_column=st.selectbox('2. 선택한 항목과 연 소득 평균의 관계를 막대그래프로 나타냅니다.',('성별', '자동차', '부동산', '소득_형태', '학력', '가족_형태', '주거_형태', '직업','나이', '가족_규모'))
        if income_column == '나이' or income_column == '가족_형태':
            for col1, col2 in [[income_column, '소득']]:
                df = train.copy()
                df = df.groupby([col1], as_index=False).mean()
