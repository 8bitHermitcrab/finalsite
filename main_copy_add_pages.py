...
def main():
    ...
    # else:
        ...
        # # 관리자 페이지
        # is_admin = st.sidebar.button("관리자")
        # if is_admin:
        #     # 로그인
        #     st.sidebar.text_input("아이디", key='input_id')
        #     st.sidebar.text_input("비밀번호", type="password", key='input_pwd')
        #     if login(st.session_state['input_id'], st.session_state['input_pwd']) == True:
        #         pass
        #     else:
        #         st.sidebar.error("관리자 페이지를 보시려면 로그인을 해주세요.")
...
def login(input_id, input_pwd):
    ...
    hashed_passwords = stauth.Hasher(input_pwd).generate()
    # authenticator = stauth.Authenticate(names,usernames,hashed_passwords,
    authenticator = stauth.Authenticate(names,input_id,hashed_passwords,
    'some_cookie_name','some_signature_key',cookie_expiry_days=30)
    # name, authentication_status, username = authenticator.login('Login','main')
    name, authentication_status, input_id = authenticator.login('Login','main')
