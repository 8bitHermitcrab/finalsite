...
def main():
    ...
    #     page = st.sidebar.radio("페이지를 선택해주세요", tuple(PAGES2.keys()), format_func=str.capitalize)
    #     PAGES2[page]()
...
def login():
    ...
    # if authentication_status:
    # return PAGES2[st.sidebar.radio("기업전용 카테고리", tuple(PAGES2.keys()), format_func=str.capitalize)]()
    # PAGES2[st.sidebar.radio("기업전용 카테고리",tuple(PAGES2.keys()), format_func=str.capitalize)]
...
def total_graph():
    ...
    # for col1,col2 in [['능력','신용도']]:
    #     df=train.copy()
    #     df['능력'] = df['소득'] / (df['나이']*365 + df['고용연수_r']*365)
    #     df['신용도'] = df['신용도'].astype(str)
    #     df = df.groupby([col1,col2], as_index=False).count()

    #     # df = df.sort_values(by='Unnamed: 0')
    #     df.loc[df['능력'] > 60, '능력'] = 60
    #     fig = px.histogram(df, x="능력", 
    #     # histnorm='Unnamed: 0',
    #     color='신용도')
    #     st.plotly_chart(fig)
...
# from plotly.subplots import make_subplots
    ...
    #     trace0 = go.Histogram(x=df['능력'],
    #     # y=df[(df['신용도']=='0') & (df['Unnamed: 0'])],
    #      nbinsx=30)
    #     trace1 = go.Histogram(x=df['능력'],
    #     # y=df[(df['신용도']=='1') & (df['Unnamed: 0'])],
    #      nbinsx=30)
    #     trace2 = go.Histogram(x=df['능력'],
    #     # y=df[(df['신용도']=='2') & (df['Unnamed: 0'])],
    #      nbinsx=30)
    ...
    # with column1:
    #     for col1,col2 in [['직업','신용도']]:
    #         df = train.copy()
    #         # df = df.groupby(by=[col,'신용도']).count().reset_index()
    #         df['신용도'] = df['신용도'].astype(str)
    #         df = df.groupby([col1,col2], as_index=False).count()
    #         df = df.sort_values(by='Unnamed: 0')
    #         # df = df.sort_values(by=df['직업'].value_counts())
    #         fig = px.bar(df, 
    #         # width=800, height=600,
    #         x=col1, y="Unnamed: 0",
    #         title=str('Bar chart: '+col1+' & 신용도 count'),
    #         barmode='group',color="신용도", labels={col1:'직업 유형','Unnamed: 0':'인원'})
    ...
    #         df = train.copy()
    #         df = df.groupby(by=[col,'신용도']).count().reset_index()
    #         df = df[['직업','신용도','Unnamed: 0']]
    #         df.rename(columns = {'Unnamed: 0' : '인원'}, inplace = True)
    #         fig = go.Figure(data=[go.Table(
    #             header=dict(values=list(df.columns),
    #                         fill_color='paleturquoise',
    #                         align='left'),
    #             cells=dict(values=[df.직업, df.신용도, df.인원],fill_color='lavender',align='left'))])
...
def total_graph2():
    st.subheader('임시1 total_graph2와 연결됨')
def total_graph3():
    st.subheader('임시2 total_graph2와 연결됨')
def total_graph4():
    st.subheader('신용도와 연관된 전체 표 보기')
    st.text('*신용도는 0등급, 1등급, 2등급으로 분류되며 숫자가 낮을수록 신용도가 높습니다.')
    st.text('*전체 화면을 권장하며, 신용도를 클릭하시면 더욱 자세히 볼 수 있습니다.')
    ...
    if income_column == '나이' or income_column == '가족 형태':
        ...
        st.plotly_chart(fig)
def total_graph5():
    st.subheader('임시4 total_graph2와 연결됨')
    # st.text('*신용도는 0등급, 1등급, 2등급으로 분류되며 숫자가 낮을수록 신용도가 높습니다.')
    # st.text('*전체 화면을 권장하며, 신용도를 클릭하시면 더욱 자세히 볼 수 있습니다.')
    # # st.title('신용카드 사용자 신용도 예측 서비스')
    # DATA_PATH = ('./data/')
